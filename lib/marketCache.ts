...
const pointerKey = (addr: string) => `pol_market_root_${addr.toLowerCase()}`
const cacheKey = (addr: string) => `pol_market_snapshot_${addr.toLowerCase()}`
const lastUploadKey = (addr: string) => `pol_market_last_upload_${addr.toLowerCase()}`
