  ...
  const startListening = useCallback(() => {
    ...
    bondingCurveTradingService.listenForCurveEvents(curveAddress, {
      onBuy: (buyer: string, ogIn: string, tokensOut: string, price: string) => {
        console.log(`🟢 Buy: ${buyer} bought ${tokensOut} tokens for ${ogIn} MATIC at ${price} MATIC/token`)
        // Refresh curve info when trades happen
        refreshCurveInfo()
      },
      onSell: (seller: string, tokensIn: string, ogOut: string, price: string) => {
        console.log(`🔴 Sell: ${seller} sold ${tokensIn} tokens for ${ogOut} MATIC at ${price} MATIC/token`)
        // Refresh curve info when trades happen
        refreshCurveInfo()
      },
      onPriceUpdate: (price: string, supply: string, ogReserve: string) => {
        console.log(`📊 Price update: ${price} MATIC/token, Supply: ${supply}, Reserve: ${ogReserve} MATIC`)
        // Update local state
        if (curveInfo) {
      ...
      onStepAdvance: (step: number, price: string) => {
        console.log(`📈 Step advanced: Step ${step}, Price: ${price} MATIC/token`)
        // Update local state
        if (curveInfo) {
          setCurveInfo(prev => prev ? {
            ...prev,
      ...
      onGraduation: (tokensSold: string, ogReserve: string, timestamp: number) => {
        console.log(`🎓 Curve graduated: ${tokensSold} tokens sold, ${ogReserve} MATIC reserve`)
        // Update local state
        if (curveInfo) {
          setCurveInfo(prev => prev ? {
            ...prev,
